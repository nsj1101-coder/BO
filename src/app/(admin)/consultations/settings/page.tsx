...
export default function ConsultationSettingsPage() {
  ...
  const fetchConfig = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/form-configs/consultation");
    if (res.ok) {
  ...
  const saveAll = async (updated: FieldConfig[]) => {
    setSaving(true);
    await fetch("/api/form-configs/consultation", {
      method: "PUT",
  ...
  const handleSave = async () => {
    ...
    const field: FieldConfig = { ...form, options: form.options };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#191F28]">상담 폼 필드 설정</h3>
          <p className="text-sm text-gray-500 mt-1">상담 신청 폼에서 수집할 필드를 관리합니다.</p>
        </div>
        <button
          onClick={openCreate}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.key} onChange={(e) => setForm({ ...form, key: e.target.value })} placeholder="예: symptom, preferred_time" disabled={editIndex !== null} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3182F6]/30 focus:border-[#3182F6] transition-all disabled:bg-gray-50 disabled:text-gray-400" />
              </div>
              <div>
