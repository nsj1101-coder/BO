  ...
  if (!item) {
    return (
      <div className="text-center py-32">
        ...
        <Link href="/inquiries" className="text-sm text-[#3182F6] mt-4 inline-block hover:underline">목록으로</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        <Link href="/inquiries" className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
          <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <h3 className="text-xl font-bold text-[#191F28]">문의 상세</h3>
      </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={label} className="grid grid-cols-[140px_1fr] items-center gap-4">
              ...
              <p className="text-sm text-[#191F28]">{value || "-"}</p>
            </div>
          ))}
          <div className="grid grid-cols-[140px_1fr] items-start gap-4">
            <label className="text-sm font-semibold text-gray-500">내용</label>
            <p className="text-sm text-[#191F28] whitespace-pre-wrap">{item.content || "-"}</p>
          </div>
        </div>
