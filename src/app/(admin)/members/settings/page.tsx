...
interface FieldForm {
  fieldKey: string;
  label: string;
  fieldType: MemberField["fieldType"];
  required: boolean;
  options: string;
}
...
const fieldTypeLabels: Record<MemberField["fieldType"], string> = {
  text: "텍스트",
  email: "이메일",
  phone: "전화번호",
  textarea: "장문",
...
const fieldTypeBadgeColors: Record<MemberField["fieldType"], string> = {
...
export default function MemberSettingsPage() {
  ...
  const fetchFields = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/member-fields");
    if (res.ok) {
  ...
  const openEdit = (field: MemberField) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const url = editId ? `/api/member-fields/${editId}` : "/api/member-fields";
    const method = editId ? "PUT" : "POST";
    const body = {
      ...form,
      sortOrder: editId
        ? fields.find((f) => f.id === editId)?.sortOrder ?? fields.length
        : fields.length,
    };
  ...
  const handleDelete = async (id: number) => {
    await fetch(`/api/member-fields/${id}`, { method: "DELETE" });
  ...
  const moveField = async (index: number, direction: "up" | "down") => {
    ...
    setFields(reordered);
    await fetch("/api/member-fields/reorder", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#191F28]">회원 가입폼 필드 설정</h3>
          <p className="text-sm text-gray-500 mt-1">회원가입 시 수집할 필드를 관리합니다.</p>
        </div>
        <button
          onClick={openCreate}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">옵션</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {fields.map((field, index) => (
                <tr key={field.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-mono text-gray-600">{field.fieldKey}</td>
                  <td className="px-6 py-4 text-sm font-medium text-[#191F28]">{field.label}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2.5 py-1 text-xs font-semibold rounded-lg ${fieldTypeBadgeColors[field.fieldType]}`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium text-[#3182F6] bg-[#3182F6]/10 rounded-lg hover:bg-[#3182F6]/20 transition-colors"
                      >
                        수정
                      </button>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-8">
            <h4 className="text-lg font-bold text-[#191F28] mb-6">
              {editId ? "필드 수정" : "필드 추가"}
            </h4>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="예: address, company"
                  disabled={!!editId}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3182F6]/30 focus:border-[#3182F6] transition-all disabled:bg-gray-50 disabled:text-gray-400"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3182F6]/30 focus:border-[#3182F6] transition-all"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, fieldType: e.target.value as MemberField["fieldType"] })}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3182F6]/30 focus:border-[#3182F6] transition-all"
                >
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${form.required ? "bg-[#3182F6]" : "bg-gray-200"}`}
                >
            ...
            <div className="flex justify-end gap-3 mt-8">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-5 py-2.5 text-sm font-semibold text-white bg-[#3182F6] rounded-xl hover:bg-[#1B64DA] disabled:opacity-50 transition-colors"
              >
                {saving ? "저장 중..." : "저장"}
              </button>
            </div>
